...
def nsga2(number_gen, num_pop):
    ...
    for gen in range(1, NGEN + 1):
        ...
        file_gen = join(dirname(__file__), "store", "population_" + date_time + ".txt")
        with open(file_gen, 'w') as f:
            f.write(str(pop))
        sys_output.print_sub_tit("Generation %s is save in %s " % (gen, file_gen))

    file = join(dirname(__file__), "population.txt")
    sys_output.print_sub_tit("Final population is saved in %s " % (file))
    with open(file, 'w') as f:
        for line in pop:
            f.write(str(line) + "\n")
    return pop
...
def select_pop_DT(pop):
    ...
    now = datetime.now()
    date_time = now.strftime("%m_%d_%Y__%H%M%S")
    file_name_cvs = join(dirname(__file__), "store", "DT_data" + date_time + ".cvs")
